...
This maintainer guide turns a reviewed commit into a traceable KPR release. Source identity, validation, packaging, provenance, and publication must describe the same candidate.

> [!IMPORTANT]
> KPR must not publish a functional release until the first-installable-release gate in `INSTALLATION.md` is complete. This process is a readiness contract, not evidence that a package exists.

## 🧭 Release profile

| Property | Requirement |
| --- | --- |
| Project maturity | Development; no supported installable release |
...
- The public API, dependency order, and compatibility boundary are frozen.
- A supported installation route replaces the current developer-evaluation-only guidance.
...
For the first functional release, treat every missing gate as blocking. Never create a tag merely to imply maturity.
...
- `VERSION`
- `CHANGELOG.md`
- public VBA module headers once stabilized
- README status and compatibility claims
- calculation-contract documentation
...
- Validate exported VBA structure and naming.
- Check that documented public contracts match the sources.
- Scan for credentials, private data, stale examples, and unresolved development markers.

Capture commands, tool versions, timestamps, and complete results. Rerun affected gates after any change.

## 6. Certify in Excel

- Import the exact candidate source into a clean workbook.
- Run **Debug → Compile VBAProject**.
- Run the complete KPR regression suite once it exists.
- Independently validate financially or numerically sensitive results against documented references.
...
- No artifact until the installable-release gate is satisfied
- Future reproducible workbook or add-in
- Optional source archive bound to the same tag
...
- artifact hashes when packaging begins
- version and tag
